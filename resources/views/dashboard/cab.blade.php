...
@section('scripts')
    ...
    <script>
        ...
        function calculateRideEarnings(ride, intRegex, floatRegex) {
            let price = parseFloat(ride.subTotal) || 0;
            let minprice = price;
            let discount = parseFloat(ride.discount) || 0;
            ...
            let tax = 0;
            if (ride.taxType && ride.tax != undefined) {
                tax = ride.taxType === 'percentage' ? (ride.tax * minprice) / 100 : parseFloat(ride.tax);
            }
            price += isNaN(tax) ? 0 : tax;

            if (ride.deliveryCharge) price += parseFloat(ride.deliveryCharge);

            let commission = 0;
            if (ride.adminCommission && ride.adminCommission > 0) {
                commission = ride.adminCommissionType === 'percentage' ?
                    (price * parseFloat(ride.adminCommission)) / 100 :
                    parseFloat(ride.adminCommission);
            }
