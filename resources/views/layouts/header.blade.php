    ...
    <a class="navbar-brand" href="<?php echo URL::to('/'); ?>">
        <b>
            <img src="{{ asset('/images/logo_web.png') }}"
                onerror="this.onerror=null; this.src='{{ asset('/images/logo_web.png') }}';" alt="homepage"
                class="dark-logo" width="100%" id="logo_web">
            <img src="{{ asset('images/logo-light-icon.png') }}"
                onerror="this.onerror=null; this.src='{{ asset('images/logo-light-icon.png') }}';" alt="homepage"
                class="light-logo">
        </b>
    </a>
    <div class="sidebar-toggle">
        <span class="nav-item mb-toggle">
            <a class="nav-link nav-toggler hidden-md-up text-muted waves-effect waves-dark" href="javascript:void(0)"><i
                ...
                <div class="dropdown-menu dropdown-setting scale-down sidebar-nav">
                    <ul class="sidebarnav">
                        @if (in_array('global-setting', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/globals') !!}"><i class="mdi mdi-web"></i>
                                    {{ trans('lang.app_setting_globals') }}</a></li>
                        @endif
                        @if (in_array('currency', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/currencies') !!}"><i
                                        class="mdi mdi-currency-usd"></i> {{ trans('lang.currency_plural') }}</a></li>
                        @endif
                        @if (in_array('business-model', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/businessModel') !!}"><i
                                        class="mdi mdi-domain"></i> {{ trans('lang.business_model_settings') }}</a></li>
                        @endif
                        @if (in_array('tax', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('tax') !!}"><i class="mdi mdi-database"></i>
                                    {{ trans('lang.tax_setting') }}</a></li>
                        @endif
                        @if (in_array('payment-method', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/payment/payme') !!}"><i
                                        class="mdi mdi-credit-card"></i> {{ trans('lang.app_setting_payment') }}</a></li>
                        @endif
                        @if (in_array('radius', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/radiusConfiguration') !!}"><i
                                        class="mdi mdi-map-marker-radius"></i> {{ trans('lang.radios_configuration') }}</a></li>
                        @endif
                        @if (in_array('app-banners-setting', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/banners') !!}"><i
                                        class="mdi mdi-page-layout-body"></i> {{ trans('lang.app_setting_banners') }}</a></li>
                        @endif
                        @if (in_array('scheduleOrderNotification', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/scheduleOrderNotification') !!}"><i
                                        class="mdi mdi-bell-outline"></i>
                                    {{ trans('lang.schedule_order_notification_title') }}</a></li>
                        @endif
                        @if (in_array('delivery-charge', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/deliveryCharge') !!}"><i
                                        class="mdi mdi-truck-delivery"></i> {{ trans('lang.delivery_charge') }}</a></li>
                        @endif
                        @if (in_array('document-verification', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/documentVerification') !!}"><i
                                        class="mdi mdi-file-document"></i> {{ trans('lang.document_verification') }}</a></li>
                        @endif
                        @if (in_array('language', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/languages') !!}"><i
                                        class="mdi mdi-translate"></i> {{ trans('lang.languages') }}</a></li>
                        @endif
                        @if (in_array('special-offer', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/specialOffer') !!}"><i
                                        class="mdi mdi-percent"></i> {{ trans('lang.special_offer') }}</a></li>
                        @endif
                        @if (in_array('settings-maintenance', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('settings/app/maintenance') !!}"><i
                                        class="mdi mdi-settings"></i> {{ trans('lang.maintenance_mode_settings') }}</a></li>
                        @endif
                        @if (in_array('terms', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('termsAndConditions') !!}"><i class="mdi mdi-file"></i>
                                    {{ trans('lang.terms_and_conditions') }}</a></li>
                        @endif
                        @if (in_array('privacy', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('privacyPolicy') !!}"><i class="mdi mdi-file-check"></i>
                                    {{ trans('lang.privacy_policy') }}</a></li>
                        @endif
                        @if (in_array('home-page', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('homepageTemplate') !!}"><i
                                        class="mdi mdi-page-layout-body"></i> {{ trans('lang.homepageTemplate') }}</a></li>
                        @endif
                        @if (in_array('footer', $role_has_permission))
                            <li><a class="nav-link" href="{!! url('footerTemplate') !!}"><i
                                        class="mdi mdi-page-layout-footer"></i> {{ trans('lang.footer_template') }}</a></li>
                        @endif
                    </ul>
                </div>
            </li>
                ...
                <div class="dropdown-menu dropdown-setting scale-down sidebar-nav">
                    <ul class="sidebarnav">
                        ...
                        @if (in_array('on-board', $role_has_permission))
                            <li><a class="waves-effect waves-dark nav-link onboard_menu" href="{!! url('on-board') !!}"
                                    aria-expanded="false">
                        ...
                        @if (in_array('email-template', $role_has_permission))
                            <li><a class="waves-effect waves-dark nav-link" href="{!! url('email-templates') !!}"
                                    aria-expanded="false">
    ...
    <div class="navbar-nav my-lg-0 multi-service-nav">
        <div class="nav-item dropdown" id="activeSection">
            <a class="nav-link dropdown-toggle waves-effect waves-dark" href="#" id="activeSectionLink"
                ...
                <img src="{{ asset('/images/logo-light-icon.png') }}" id="activeSectionLogo"
